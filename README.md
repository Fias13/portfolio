...
- **Backend (`server/`)** → [Render](https://render.com). New Web Service pointed at this repo, root directory `server`, build command `npm install --include=dev && npx prisma generate && npm run build`, start command `npm run start`. Set every variable from `server/.env.example` (including the Supabase Storage ones below), and run `npx prisma migrate deploy && npm run seed` once via the Render Shell (or as a pre-deploy command) to set up tables and the admin user.
  - **`--include=dev` is required**: with `NODE_ENV=production` set (as recommended below), plain `npm install` skips `devDependencies` — including `typescript` and the `@types/*` packages the build needs to compile. `--include=dev` forces them to install anyway, while the app still runs in production mode at runtime.
  - **Use the Supabase _session pooler_ connection string** (port `5432`, not the `6543` transaction pooler) for `DATABASE_URL` on a persistent server like Render — the transaction pooler doesn't support the advisory locks Prisma Migrate needs, so `prisma migrate deploy` hangs on it. The session pooler works for both migrations and normal app queries.
